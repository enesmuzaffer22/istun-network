...
/**
 * @openapi
 * /api/bridgeprojectsimpact:
 *   get:
 *     tags: [Bridge Projects Impact]
 *     summary: Bridge Projects Impact verilerini getir (tek kayıt)
 *     responses:
 *       200:
 *         description: Impact verileri veya boş durumda varsayılanlar
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *   post:
 *     tags: [Bridge Projects Impact]
 *     summary: Bridge Projects Impact verilerini oluştur (ilk kez)
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 *       required: true
 ...
 *           schema:
 *             type: object
 *             required: [volunteer_student, donated_institution, active_project, volunteer_hour]
 *             properties:
 *               volunteer_student:
 *                 type: integer
 *               donated_institution:
...
router.post("/", protect, isAdmin, async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({ message: "Tüm impact verileri zorunlu." });
    }
    ...
    if (
      ...
      return res.status(400).json({ message: "Tüm değerler sayı olmalıdır." });
    }

    // Mevcut kayıt var mı kontrol et
    const existingSnapshot = await db
      .collection("bridgeprojectsimpact")
      .limit(1)
      .get();

    if (!existingSnapshot.empty) {
      return res.status(400).json({
        message: "Impact verileri zaten mevcut. Güncellemek için PUT kullanın.",
      });
    }
    ...
    res.status(201).json({
      id: docRef.id,
      message: "Bridge Projects Impact verileri eklendi.",
    });
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
 ...
 *   put:
 *     tags: [Bridge Projects Impact]
 *     summary: Bridge Projects Impact verilerini güncelle
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 *       required: true
  ...
  try {
    ...
    if (snapshot.empty) {
      return res.status(404).json({
        message: "Impact verileri bulunamadı. Önce POST ile oluşturun.",
      });
    }
    ...
    if (volunteer_student !== undefined) {
      ...
      if (isNaN(volunteerStudent)) {
        return res
          .status(400)
          .json({ message: "volunteer_student sayı olmalıdır." });
      }
    ...
    if (donated_institution !== undefined) {
      ...
      if (isNaN(donatedInstitution)) {
        return res
          .status(400)
          .json({ message: "donated_institution sayı olmalıdır." });
      }
    ...
    if (active_project !== undefined) {
      ...
      if (isNaN(activeProject)) {
        return res
          .status(400)
          .json({ message: "active_project sayı olmalıdır." });
      }
    ...
    if (volunteer_hour !== undefined) {
      ...
      if (isNaN(volunteerHour)) {
        return res
          .status(400)
          .json({ message: "volunteer_hour sayı olmalıdır." });
      }
 ...
 *   delete:
 *     tags: [Bridge Projects Impact]
 *     summary: Bridge Projects Impact verilerini sil
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
...
router.delete("/", protect, isAdmin, async (req, res) => {
  try {
    ...
    if (snapshot.empty) {
      return res.status(404).json({ message: "Impact verileri bulunamadı." });
    }
