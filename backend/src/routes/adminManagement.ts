...
import { setAdminRole, removeAdminRole, getAdminRole } from "../utils/adminUtils";
...
router.post("/set-role", protect, isSuperAdmin, async (req, res) => {
    try {
        ...
        if (!['super_admin', 'content_admin'].includes(role)) {
            return res.status(400).json({ message: "Geçersiz rol. 'super_admin' veya 'content_admin' olmalıdır." });
        }
...
router.post("/remove-role", protect, isSuperAdmin, async (req, res) => {
    try {
        ...
        if (success) {
            ...
            res.status(500).json({ message: "Admin rolü kaldırılırken hata oluştu." });
        }
    } catch (error: any) {
        res.status(500).json({ message: "Sunucu hatası: " + error.message });
    }
