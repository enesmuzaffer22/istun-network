 ...
 *   get:
 *     tags: [SocialImpactScores]
 *     summary: Social Impact Scores verilerini getir (tek kayıt)
 *     responses:
 *       200:
 *         description: Social Impact Scores tek kayıt
 *       500:
 ...
 *   post:
 *     tags: [SocialImpactScores]
 *     summary: Social Impact Scores verilerini ilk kez oluştur
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 *       required: true
 ...
 *           schema:
 *             type: object
 *             required: [social_impact_score, number_of_people_reached, social_projects, awards]
 *             properties:
 *               social_impact_score:
 *                 type: string
  ...
  try {
    ...
    if (!existingSnapshot.empty) {
      return res
        ...
        .json({
          message:
            "Social Impact Scores verileri zaten mevcut. Güncellemek için PUT kullanın.",
        });
    }
    ...
    res
      ...
      .json({
        id: docRef.id,
        message: "Social Impact Scores verileri eklendi.",
      });
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
 ...
 *   put:
 *     tags: [SocialImpactScores]
 *     summary: Social Impact Scores verilerini güncelle
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 *       required: true
  ...
  try {
    ...
    if (snapshot.empty) {
      return res
        ...
        .json({
          message:
            "Social Impact Scores verileri bulunamadı. Önce POST ile oluşturun.",
        });
    }
 ...
 *   delete:
 *     tags: [SocialImpactScores]
 *     summary: Social Impact Scores verilerini sil
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
