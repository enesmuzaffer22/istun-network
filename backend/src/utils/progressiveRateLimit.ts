...
export class ProgressiveRateLimit {
  ...
  constructor() {
    ...
    this.config = {
      ...
      escalationWindowMs: parseInt(
        process.env.BAN_ESCALATION_WINDOW_MS || "604800000"
      ), // 7 gün
    };
  ...
  private cleanupExpiredEntries(): void {
    ...
    if (keys.length > maxCacheEntries) {
      console.warn(
        `⚠️ Cache limit aşıldı: ${keys.length} entry. Temizleme yapılıyor...`
      );
      this.forceCleanup();
      return;
    }
  ...
  checkRateLimit(ip: string): {
    ...
    if (banCheck.banned) {
      ...
      return {
        ...
        message: `IP adresiniz ${remainingMinutes} dakika daha engellenmiştir. Seviye: ${
          banCheck.banInfo?.level || 0
        }`,
      };
    }
  ...
  private handleRateLimitViolation(
    ...
    const existingViolations = this.cache.get<{
      count: number;
      firstViolation: number;
      lastViolation: number;
    }>(violationsKey);
    ...
    if (existingViolations) {
      violationCount = Math.min(
        existingViolations.count + 1,
        this.config.maxBanLevel
      );
    ...
    this.cache.set(
      violationsKey,
      { count: violationCount, firstViolation, lastViolation },
      Math.ceil(this.config.escalationWindowMs / 1000)
    );
    ...
    return {
      ...
      resetTime: banUntil,
      banInfo,
      message: `Rate limit aşıldı! IP adresiniz ${durationText} süreyle engellenmiştir. (Seviye: ${newLevel}, Toplam ihlal: ${violationCount})`,
    };
  }
}
