...
function RoadmapDetailPage() {
  ...
  useEffect(() => {
    fetch("/src/pages/roadmapsData.json")
      .then((res) => res.json())
      .then((data) => {
  ...
  if (!roadmap) return <div>Yol haritası bulunamadı.</div>;
  ...
  return (
    <div className="flex flex-col gap-8 2xl:px-[120px] px-4 py-12 md:py-[90px]">
      ...
      <div className="text-gray-400 text-base mb-4">{roadmap.date}</div>
      <div className="text-gray-600 text-lg mb-4">
        <ReactMarkdown components={markdownComponents}>{roadmap.roadmap_description}</ReactMarkdown>
      </div>
      <div className="text-base sm:text-lg text-gray-800">
        <ReactMarkdown components={markdownComponents}>{roadmap.roadmap_content}</ReactMarkdown>
      </div>
    </div>
  );
}
