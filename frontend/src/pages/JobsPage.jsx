...
function JobsPage() {
  ...
  useEffect(() => {
    if (!loading) {
      ...
      // Başlık her zaman animasyonlu görünsün
      tl.fromTo(
        titleRef.current,
        { opacity: 0, y: 30 },
        { opacity: 1, y: 0, duration: 0.8, ease: "power2.out" }
      );
      // İş kartları sadece veri varsa animasyonlu görünsün
  ...
  return (
    <div className="flex flex-col gap-12 2xl:px-[120px] px-4 py-12 md:py-[90px]">
        ...
        {jobsData.length === 0 ? (
          <div className="text-center py-12">
            <div className="flex flex-col items-center gap-4 text-gray-500">
              <i className="bi bi-briefcase text-6xl mb-4"></i>
              <h3 className="text-xl font-semibold text-gray-700">
                Henüz İş İlanı Bulunmuyor
              </h3>
              <p className="text-gray-500 max-w-md">
                Şu anda görüntülenecek iş ilanı bulunmamaktadır. Yakında yeni
                fırsatlar eklenecektir.
              </p>
            </div>
          </div>
        ) : (
      ...
      {/* Pagination Controls - Sadece veri varsa göster */}
      {jobsData.length > 0 && (
